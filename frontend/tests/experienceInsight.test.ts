...
assert(rows.length === 4, "体验服摘要应该展示更新、爆料、资格、当前节点四项")
assert(rows.some((row) => row.label === "更新内容" && row.value.includes("核电站")), "应该展示更新内容")
assert(rows.some((row) => row.label === "爆料内容" && row.value.includes("资源点")), "应该展示爆料内容")
