...
async def init_db():
    ...
    async with engine.begin() as conn:
        ...
        try:
            ...
            pass  # 列已存在则忽略
        try:
            await conn.run_sync(lambda c: c.exec_driver_sql("ALTER TABLE platform_search_configs ADD COLUMN proxy_url VARCHAR(512)"))
        except Exception:
            pass
        try:
        ...
        await conn.run_sync(lambda c: c.exec_driver_sql("CREATE INDEX IF NOT EXISTS ix_external_monitor_records_source_external ON external_monitor_records (source_key, external_id)"))
